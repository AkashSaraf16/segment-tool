...
export default function SegmentEdit() {
	...
	useEffect(() => {
		...
		for (let i = 0; i < hullPoints.length - 1; i++) {
			const line = CustomLine([hullPoints[i][0], hullPoints[i][1], hullPoints[i + 1][0], hullPoints[i + 1][1]])
			const circleStart = CustomCircle([hullPoints[i][0], hullPoints[i][1]], intersectingLines, lineLayer, buttonRef)
